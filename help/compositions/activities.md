...
De plus, vous pouvez définir l’expiration des données pour l’audience. L’expiration des données détermine le nombre de jours après lesquels l’appartenance à l’audience expirera. L’expiration des données peut prendre entre 1 et 90 jours. Par défaut, cette valeur est définie sur 30.
...
>abstract="L’activité Enregistrer les champs permet d’enrichir les schémas Experience Platform en fédérant les données d’entrepôts externes, ce qui permet d’enrichir les schémas Experience Platform avec des attributs supplémentaires. "

>[!CONTEXTUALHELP]
>id="dc_orchestration_savedataset_primaryidentitynamespace"
>title="Principal champ d’espace de noms d’identification"
...
>title="Mode de mise à jour de l’enregistrement des champs"
...
>abstract="Le mode de mise à jour complète met à jour l’ensemble des attributs dans les schémas sélectionnés."
...
>title="Sélectionner le jeu de données"
...
>title="Clé primaire"
>abstract="Clé primaire du schéma relationnel. Cette valeur garantit le caractère unique des éléments dans les jeux de données en empêchant l’ingestion d’enregistrements en double."

>[!CONTEXTUALHELP]
>id="dc_orchestration_savedataset_versiondescriptor"
>title="Descripteur de version"
>abstract="Descripteur de version du schéma relationnel. Cette valeur permet de déterminer la propriété prioritaire si plusieurs valeurs partagent la même clé primaire. Ceci garantit que la mise à jour appliquée est bien la dernière."

>[!CONTEXTUALHELP]
>id="dc_orchestration_savedataset_timestampdescriptor"
>title="Descripteur d’horodatage"
>abstract="Descripteur de date et heure du schéma relationnel. Cette valeur permet de définir l’heure de l’événement pour la commande et n’existe que si vous utilisez des données de série temporelle."
...
>[!IMPORTANT]
>
>Si **aucun** upsert n’est activé pour le jeu de données sélectionné, les données sont **remplacées**. Pour savoir comment activer l’upsert pour vos jeux de données, consultez le [guide d’activation de l’upsert](https://experienceleague.adobe.com/fr/docs/experience-platform/catalog/datasets/enable-upsert).
